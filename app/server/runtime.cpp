...
Value parse_multipart_json_field(const std::string & field_name, const std::string & value) {
    ...
    if (field_name == "busy_timeout_ms") {
        try {
            return Value::make_number(std::stoi(value));
...
HttpResponse ServerState::handle_speech_multipart(const std::string & body_text, const std::string & boundary) {
    ...
    const auto busy_timeout_ms = parse_busy_timeout_override(payload.body);
    const auto timed_result = model.task.mode == engine::runtime::RunMode::Streaming
        ? run_streaming_model(model, request, {}, busy_timeout_ms)
        : run_model(model, request, busy_timeout_ms);
    const auto & audio = select_audio_output(timed_result.result);
...
HttpResponse ServerState::handle_speech_stream(
    ...
    auto stream_body = [this, model_ptr, request, multipart_cleanup, busy_timeout_ms](HttpStreamWriter & writer) {
    ...
    return chunked_audio_response([this, model_ptr, request, multipart_cleanup, busy_timeout_ms](HttpStreamWriter & writer) {
        bool wrote_audio = false;
